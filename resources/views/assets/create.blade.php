<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center bg-gradient-to-r from-purple-500 to-purple-800 p-6 shadow-xl text-white rounded-lg">
            <h1 class="text-3xl font-bold">{{ __('Create New Asset') }}</h1>
            <div class="flex justify-end">
                <a href="{{ route('assets.index') }}" class="bg-white hover:bg-gray-100 text-purple-600 font-bold py-2 px-4 rounded-full shadow transition-transform transform hover:scale-110 duration-300">
                    &larr; Back
                </a>
            </div>
        </div>
